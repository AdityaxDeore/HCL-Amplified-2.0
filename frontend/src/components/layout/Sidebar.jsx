...
import { Home, Map, Sparkles, Search, BookOpen, BarChart2, Mic, Target, Settings, Sparkle, ChevronRight, Award } from 'lucide-react'

const mainNavItems = [
  { label: 'Dashboard', icon: Home, path: '/dashboard' },
  { label: 'My Roadmap', icon: Map, path: '/roadmap' },
  { label: 'Job Readiness', icon: Award, path: '/readiness' },
...
export default function Sidebar() {
  ...
  const isPathActive = (path) => {
    // Exact match for dashboard to avoid matching /dashboard-something (unlikely but safe)
    if (path === '/dashboard') return location.pathname === '/dashboard';
    // Prefix match for all other routes so nested routes keep the parent active
  ...
  return (
    <aside className="group w-24 hover:w-[280px] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col items-start py-8 bg-white/80 backdrop-blur-xl border-r border-white/40 z-50 flex-shrink-0 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      {/* Logo */}
      <Link to="/dashboard" className="mb-10 px-6 flex items-center gap-4 w-full cursor-pointer relative">
        ...
        {mainNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`relative flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 w-full overflow-hidden group/item ${
                isActive
